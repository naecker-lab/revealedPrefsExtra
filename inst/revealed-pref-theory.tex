...
\todo{This is still a bit confusing. Please clarify.}
\end{definition}
...
\todo{This citation is not showing up. Did you forget to commit the bib file?}
\end{remark}
...
\todo{Note that using the command version of max (with a slash) results in properly un-italicized text for functions in math mode.  Go ahead and fix throughout.} 
...
Let $A_d$ be the adjacency matrix of $G_d$ such that $A_{d_{i,j}}=1$ if there exists an edge between $X_i$ and $X_j$, i.e. $X_i$ is directly revealed preferred to $X_j$. \todo{Define what an adjacency matrix should look like. \checkmark}. To be rigorous, we can generate $A_d$ from our data set in the following manner. First, create the following $(k\times k)$ matrix from $\mathcal{X}$ and $\mathcal{P}$:
...
We know that the number of directed walks from any vertex $i$ to vertex $j$ of length $k$ is given by $A^k_d$ (the $k^{th}$ power of $A_D$) \todo{Make clear this is the $k$th power. \checkmark} for adjacency matrix $A_d$ (this can be proven by induction). 
...
\begin{theorem}
\label{thm:WGARP}
A data set $(\mathcal{X}, \mathcal{P})$ passes WGARP if and only if the main diagonal of $A_d \times A_s$ is a vector of 0. \todo{Note theorem environment. Please use for other theorems.  Also note label, which allows you to refer to theorems like this: Theorem \ref{thm:WGARP}. \checkmark}
\end{theorem}
...
Hence, $\forall i, j\in\{1,\ldots,k\}$ such that $i\not=j$, $(A_d\times A_s)_{i,i}=0 \iff (A_{d_{i,j}}=1 \implies A_{s_{j,i}}=0\not=1)$. Note the first statement simply means the main diagonal of $A_d\times A_s$ is the zero vector and the second statement is the condition for passing WGARP. We then have ``passing WGARP'' \todo{Note the double backticks gives quotation marks in the right direction. Fix throughout. \checkmark}$\iff$ ``main diagonal of $A_d\times A_s$ is a zero vector''. Thus our claim is proven (since $A\iff B$ is the same as $B\iff A$).
\end{proof}
...
In our proof of WGARP, we see that $\forall i, j\in\{1,\ldots,k\}$ such that $i\not=j$, $(A_d\times A_s)_{i,i}=0 \iff (A_{d_{i,j}}=1 \implies A_{s_{j,i}}=0\not=1)$.\todo{Notation here is confusing. Please clarify.} There are two changes we need to make to this statement to fit our GARP claim. 
...
\todo[inline]{Why $\geq 1$ here? Can't $W$ be any non-negative number? 

\textbf{A:} We want our weights to be defined $\frac{P_iX_i}{P_iX_j}$. But the domain is $E_d$, and for $(X_i,X_j)\in E_d$, we need $P_iX_i\geq P_i X_j$. Hence, $\frac{P_iX_i}{P_iX_j}\geq1$, which is the codomain / range of our function. The case for $W_s$ is similar in that it is a strict inequality.}
\bigskip
...
where $z_{i,j}$ is the $(i,j)^{th}$ element of $Z$. note that this is similar with matrix multiplication in that we consider the $i^{th}$ row of $X$ and $j^{th}$ column of $Y$, but instead of pairwise multiplication and overall summation (i.e. dot product), we take pairwise minimum and overall maximum. \todo[inline]{It's really not clear to my why we are setting up this max-min stuff. Please give some more context.

\textbf{Answer:} The idea is that we want to, for a given $X_i$, find all the other edges it's connected to via a path. If $X_i$ and $X_j$ are connected by a path, and $(X_j,X_i)\in E_s$, there's a GARP violation. The CCEI score helps us break either that path or the $(X_j,X_i)\in E_s$ to get rid of the violation. To do this, we need to look at paths of different lengths and then find the CCEI across all lengths. This is more thoroughly explain in \textit{Corollary 3.1, 3.2} and \textit{Theorem 4-6}.}
\bigskip
...
\[
  ...
  \end{cases}
\]
  \todo[inline]{Took me a while to figure out what strongest weakest edge meant.  Are you saying take the weakest edge of each path, then take the strongest of those? Please clarify or give an example.

  \textbf{Answer:} Yes, we essentially first look at all the paths and take their minimum edge, and then take the maximum across those minimum edges. The idea here is that if we can ``break'' the that edge, we will break the all the other minimum edges and therefore the walk wouldn't exist.}
...
\[
  ...
  \end{cases}
\]
\todo[inline]{Very confused by the formula above. Needs to be broken down a lot.
