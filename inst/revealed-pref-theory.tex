...
Let $A_d$ be the adjacency matrix \todo{Define what an adjacency matrix should look like.} of $G_d$. To be rigorous, we can generate $A_d$ from our data set in the following manner. First, create the following $(k\times k)$ matrix from $\mathcal{X}$ and $\mathcal{P}$:
...
We know that the number of directed walks from any vertex $i$ to vertex $j$ of length $k$ is given by $A^k_d$ \todo{Make clear this is the $k$th power.} for adjacency matrix $A_d$ (this can be proven by induction). 
...
\begin{theorem}
\label{thm:WGARP}
A data set $(\mathcal{X}, \mathcal{P})$ passes WGARP if and only if the main diagonal of $A_d \times A_s$ is a vector of 0. \todo{Note theorem environment. Please use for other theorems.  Also note label, which allows you to refer to theorems like this: Theorem \ref{thm:WGARP}.}
\end{theorem}
...
Hence, $\forall i, j\in\{1,\ldots,k\}$ such that $i\not=j$, $(A_d\times A_s)_{i,i}=0 \iff (A_{d_{i,j}}=1 \implies A_{s_{j,i}}=0\not=1)$. Note the first statement simply means the main diagonal of $A_d\times A_s$ is the zero vector and the second statement is the condition for passing WGARP. We then have ``passing WGARP" \todo{Note the double backticks gives quotation marks in the right direction. Fix throughout.}$\iff$ "main diagonal of $A_d\times A_s$ is a zero vector". Thus our claim is proven (since $A\iff B$ is the same as $B\iff A$).
\end{proof}
