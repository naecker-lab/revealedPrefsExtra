...
\textbf{Claim 1:} The data set $(\mathcal{X}, \mathcal{P})$ passes WGARP if and only if the main diagonal of $\lambda(\Omega_d, \Omega_s)$ is a vector of $-1$'s, i.e.:
...
\begin{proof}
Recall that: If $(\mathcal{X},\mathcal{P})$ passes WGARP, then: $\forall i,j\in\{1,\dots,k\}$ where $i\not=j$, if $(X_i,X_j)\in E_d$, then $(X_j, X_i)\not\in E_s$. Putting this in context of adjacency matrices, we have: $\forall i,j\in\{1,\ldots,k\}$ such that $i\not=j$, if the $(i,j)^{th}$ element of $\Omega_d$ is greater than or equal to zero, then the $(j,i)^{th}$ element of $\Omega_s$ have to be equal to -1. Thus, we need $(\Omega_{d_{i,j}}\geq0 \implies \Omega_{s_{j,i}}=-1)$ to pass WGARP. This simply means that if there exists some edge between $X_i$ and $X_j$ in $G_d$ (no matter the weight), there cannot exist an edge between $X_j$ and $X_i$ in $G_s$. We then consider $\lambda(\Omega_d, \Omega_s)$ and its diagonal.
\bigskip

$\forall i\in \{1,\ldots,k\}$, let $\delta^i$ denote the vector of the $i^{th}$ row of $\Omega_d$ and $\xi^i$ be the vector of the $i^{th}$ column of $\Omega_s$. Then we have: $$\lambda(\Omega_d, \Omega_s)_{i,i}=max\{ min\{\delta_1^i, \xi_1^i\}, \ldots, min\{\delta_i^i, \xi_i^i\}, \ldots, min\{\delta_k^i, \xi_k^i\}\}$$
Note that all elements in $\Omega_d\in \{-1\}\cup\mathbb{R}_{\geq0}$ and all elements in $\Omega_s \{-1\}\cup\mathbb{R}_{>0}$, with $-1$ denoting no edge between two vectices and non-negative (positive in the case of $\Omega_s$) real numbers denoting the weight of an existing edge. Then $\forall i,j\in\{1,\ldots,k\}$, $\delta_j^i,\xi_j^i$ are elements in $\Omega_d$ and $\Omega_s$, respectively, and would be no less than $-1$ by construction. Let $\lambda_j^i=min\{\delta_j^i,\xi_j^i\}$ for $i,j\in\{1,\ldots,k\}$ and we say $\forall i,j\in\{1,\ldots,k\}$, $\lambda_j^i=min\{\delta_j^i,\xi_j^i\}\in\{-1\}\cup\mathbb{R}_{\geq0}$.

We then know that, $\forall i\in \{1,\ldots,k\}$, $\lambda(\Omega_d, \Omega_s)_{i,i}=max\{\lambda_1^i, \ldots, \lambda_i^i, \ldots, \lambda_k^i\}=-1$ if and only if $\forall j\in\{1,\ldots,k\}$, $\lambda_j^i=-1$ since $\lambda_j^i\in\{-1\}\cup\mathbb{R}_{\geq0}$ for all $j$. Further, $\forall j\in\{1,\ldots,k\}$, $\lambda_j^i=-1$ if and only if $min\{\delta_j^i,\xi_j^i\}=-1$. Here we note that $\lambda_i^i$ must equal to $-1$ since $\delta_i^i=\xi_i^i=-1$ (there are no loops in $\Omega_d$ and $\Omega_s$) and thus $min\{\delta_i^i,\xi_i^i\}=-1$. We then have $min\{\delta_j^i,\xi_j^i\}=-1$ if and only if $(\delta_j^i\geq0 \implies \xi_j^i=-1)$, which is true since $\delta_j^i\not=1 \iff \delta_j^i\geq0$ (as $\delta_j^i\in \{-1\}\cup\mathbb{R}_{\geq0}$). By definitions of $\delta_j^i$ and $\xi_j^i$, we then have $(\delta_j\geq0 \implies \xi_j=-1)$ if and only if $(\Omega_{d_{i,j}}\geq0 \implies \Omega_{s_{j,i}}=-1)$ (as $\delta^i$ is the $i^{th}$ row of $\Omega_d$ and $\xi^i$ is the $i^{th}$ \textit{column} of $\Omega_s$). Summarizing, we have:
\begin{flalign*}
\forall i\in \{1,\ldots,k\},\ \lambda(\Omega_d, \Omega_s)_{i,i}=-1 &\iff \forall j\in\{1,\ldots,k\},\ \lambda_j^i=-1 &\\
& \iff min\{\delta_j^i,\xi_j^i\}=-1 &\\
& \iff (\delta_j^i\geq0 \implies \xi_j^i=-1) &\\
& \iff (\Omega_{d_{i,j}}\geq0 \implies \Omega_{s_{j,i}}=-1)
\end{flalign*}
We then conclude that $\forall i,j\in \{1,\ldots,k\}$ such that $i\not=j$, $\lambda(\Omega_d, \Omega_s)_{i,i}=-1 \iff (\Omega_{d_{i,j}}\geq0 \implies \Omega_{s_{j,i}}=-1)$. Note the first statement means the main diagonal of $\lambda(\Omega_d, \Omega_s)$ is a vector of $-1$'s and the second statement is the condition of passing WGARP. We then have "passing WGARP" $\iff$ "the main diagonal of $\lambda(\Omega_d, \Omega_s)_{i,i}$ is a vector of $-1$'s", which proves our claim.
\end{proof}
\bigskip

\textbf{Claim 2:} Let $\Omega_d, \Omega_s$ be such that: 
...
\end{pmatrix}
\]
and thus the data set fails WGARP. Then, $\forall i\in\{1,\ldots,k\}$, $\lambda(\Omega_d, \Omega_s)_{i,i}$ captures the lesser pairwise weight of the worst WGARP violation involving vertex $i$.

\begin{proof}
Fix an arbitrary $i\in\{1,\ldots,k\}$ and consider $\lambda(\Omega_d, \Omega_s)_{i,i}$.
\end{proof}
