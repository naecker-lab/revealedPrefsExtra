...
\textbf{Claim:} $\forall t\in \mathbb{N}_{\geq2}$,
\[
...
\[
  ...
  &\textrm{where } W\textrm{ is the weight function, as defined previously}
  \end{cases}
\]
Intuitively, we say:
\[
  ...
  & \textrm{of the smallest preference in each preference chain }
  \end{cases}
\]
...
\circled{1} If $\forall f\in\{1,\ldots,k\}$, $\Omega^d_{i,f}=-1$ or $\Omega^d_{f,j}=-1$, then $\lambda(\Omega_d,\Omega_d)_{i,j}=-1$ as $\forall i,j\in\{1,\ldots,k\}$, $\Omega^d_{i,j}\in\{-1\}\cup\mathbb{R}_{\geq0}$. Thus, if $\forall f\in\{1,\ldots,k\}$, $\Omega^d_{i,f}=-1$ or $\Omega^d_{f,j}=-1$, then $\lambda(\Omega_d,\Omega_d)_{i,j}=max\{-1,\ldots,-1\}=-1$.

\circled{2} Suppose $\exists F\subseteq\{1,\ldots,k\}$ (where $F\not=\varnothing$) such that $\forall f\in F$, $\Omega^d_{i,f}\geq0$ and $\Omega^d_{f,j}\geq0$. We then have $\lambda(\Omega_d,\Omega_d)_{i,j}=max\big\{min\{\Omega^d_{i,f},\Omega^d_{f,j}\}, -1\ |\ f\in F\big\}=max\big\{min\{\Omega^d_{i,f},\Omega^d_{f,j}\}\ |\ f\in F\big\}$.

Considering \circled{1} and \circled{2}, we have precisely the definition in our claim:

For \circled{1}, if $\nexists f$ such that $\Omega^d_{i,f}\geq0$ and $\Omega^d_{f,j}\geq0$, then we know $(X_i,X_f)\not\in E_d$ or $(X_f, X_j)\not\in E_d$ for all $f\in\{1,\ldots,k\}$. Hence, $\forall f$, there isn't an edge from $X_i$ to $X_f$ or there isn't an edge from $X_f$ to $X_j$. Thus, we know there isn't a walk of 2 edges from $X_i$ to $X_j$. We then conclude that $\lambda(\Omega_d,\Omega_d)_{i,j}=-1$ if there isn't a walk of 2 edges from $X_i$ to $X_j$.

For \circled{2}, we have that $\lambda(\Omega_d,\Omega_d)_{i,j}=max\big\{min\{\Omega^d_{i,f},\Omega^d_{f,j}\}\ |\ f\in F\big\}$, where $\varnothing\not=F\subseteq\{1,\ldots,k\}$ such that $\forall f\in F$, $\Omega^d_{i,f}\geq0$ and $\Omega^d_{f,j}\geq0$. We then know that, $\forall f\in F$, $(X_i,X_f)\in E_d$ and $(X_f,X_j)\in E_d$. For each $f$, there's a unique walk from $X_i$ to $X_j$ that consists of $\{(X_i,X_f), (X_f,X_j)\}$. Note that the weights of those edges of each walk are $\Omega^d_{i,f}$ and $\Omega^d_{f,j}$, respectively. Hence, when we take $\lambda(\Omega_d,\Omega_d)_{i,j}=max\big\{min\{\Omega^d_{i,f},\Omega^d_{f,j}\}\ |\ f\in F\big\}$, we're taking the minimum weight between two edges of each walk and taking the maximum across all walks, which is precisely our value of ``the strongest edge among the weakest edges of all walks of $2$ edges''.

Combining those, we have the following:
\[
\lambda_2(\Omega_d,\Omega_d)_{i,j}=\lambda(\Omega_d,\Omega_d)_{i,j}=
\begin{cases}
-1 & \textrm{if } X_i, X_j \textrm{ are not connected by a walk of } \\
& 2\textrm{ edges on }G_d \\
\rho_{i,j} & \textrm{such that } \rho_{i,j} \textrm{ is the weight of the} \\
& \textrm{strongest edge among the weakest edges} \\
& \textrm{of all walks of } 2\textrm{ edges from }X_i \textrm{ to } X_j \\
& \textrm{on } G_d
\end{cases}
\]
Hence, our base case proof is complete.

\textbf{Inductive Case:} Let $t=n$, we then have: 
\[
...
\begin{cases}
-1 & \textrm{if } X_i, X_j\textrm{ are not connected by a walk of }n \textrm{ edges} \\
  & \textrm{on }G_d \\
\rho_{i,j} & \textrm{such that }\rho_{i,j}\textrm{ is the weight of the strongest edge } \\
  &\textrm{among the weakest edges of all walks of } n \textrm{ edges} \\
  & \textrm{from }X_i\textrm{ to }X_j \textrm{ on } G_d
\end{cases}
\]
