...
Let $\mathcal{X}=\{X_1,\ldots,X_k\}$ and $\mathcal{P}=\{P_1,\ldots,P_k\}$ as before. Then let us set up $G_d$ and $G_s$ but define functions: $W_d:E_d \to\mathbb{R}_{\geq1}$\todo{Why $\geq 1$ here? Can't $W$ be any non-negative number?} and $W_s:E_s \to\mathbb{R}_{>1}$ so that $W(e_d)$ is the weight of edge $e_d$ and $W(e_s)$ is the weight of edge $e_s$. Let us rigorously define our weight functions.
\bigskip
