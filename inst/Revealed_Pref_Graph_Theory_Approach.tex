...
\AfriatNonCompact*
\begin{proof}
For a data set $(\mathcal{X},\mathcal{P})$ that fails GARP, define a \textbf{conflict} as a pair $(X_i,X_j)$ such that $X_i R X_j$ and $X_j R_{SD} X_i$. We say that a particular $e$ \textbf{``solves the conflict''} $X_i,X_j)$ if the conflict is no longer present in the graphs $G_{d(e)}$ and $G_{s(e)}$. That is, either the walk from $X_i$ to $X_j$ is no longer present in $G_{d(e)}$ or $(X_j,X_i)\not\in E_{s(e)}$. Lastly, we define a \textbf{violating cycle} between $G_{d}$ and $G_{s}$ as a sequence $(X^k)_{k=1}^K$, where $X^k\in\mathcal{X}$ for all $k$ and there's a walk between $X^1$ and $X^{K-1}$ in $G_d$ and $(X^{K-1},X^K)\in E_s$. Then, it's pretty clear that if data set $(\mathcal{X},\mathcal{P})$ that fails GARP, there must exist at least one violating cycle between graphs $G_d$ and $G_s$ created by the data set.

Similar to cycles in a single graph, the starting point in the violating cycle could be arbitrary. For example, using the previous set-up, there might also be a walk between $X^2$ to $X^K$ in $G_d$ and $(X^K, X^1)\in E_s$ as the two graphs $G_d$ and $G_s$ differ only on strict vs. non-strict inequality requirements in their edge sets. Thus, we further define a \textbf{unique violating cycle (UVC)} and say $(X^k)_{k=1}^K$ and $(X^j)_{j=1}^J$ are unique violating cycles if they are both violating cycles and $\left\{\textrm{unique terms in }(X^k)_{k=1}^K\right\}\not=\left\{\textrm{unique terms in }(X^j)_{j=1}^J\right\}$. In other words, two violating cycles are unique if either their initial walks on $G_d$ goes over different vertices or their final edges on $E_s$ is different. Then consider the following lemma, which is simply an extension of Lemma \ref{Lemma:Connectedness}:

\begin{lemma}\label{lemma:Connectedness2}
Suppose $(\mathcal{X},\mathcal{P})$ fails GARP and has $n$ (finite) unique violating cycles. Let $i\in\{1,\ldots,n\}$ be an arbitrary unique violating cycle (UVC) and define 
$$\mathcal{E}_{(\mathcal{X},\mathcal{P})}^i = \{e\in[0,1]\ |\ \textrm{UVC }i\textrm{ is no longer present in } G_{d(e)}\textrm{ and } G_{s(e)} \}$$
Then, $\forall i$, if $e'\in\mathcal{E}_{(\mathcal{X},\mathcal{P})}^i$, then $e\in \mathcal{E}_{(\mathcal{X},\mathcal{P})}^i$ for all $e\leq e'$.
\end{lemma}

The reasoning is similar to that of Lemma \ref{lemma:Connectedness2} so the proof is omitted here. Further, a similar line of logic as Theorem \ref{thm:Afriat_Connected} suggests the following:

\begin{corr}\label{Corr:Connectedness}
Following the set up of Lemma \ref{lemma:Connectedness2}, $\forall i\in\{1,\ldots,n\}$, $\mathcal{E}_{(\mathcal{X},\mathcal{P})}^i$ is a connected set and $0\in \mathcal{E}_{(\mathcal{X},\mathcal{P})}^i$.
\end{corr}

Note that $0\in \mathcal{E}_{(\mathcal{X},\mathcal{P})}^i$ can be easily seen from the fact that $E_{s(0)}=\varnothing$ for all data sets $(\mathcal{X},\mathcal{P})$ as one cannot possibly observe negative prices / quantity purhcased.
\end{proof}
