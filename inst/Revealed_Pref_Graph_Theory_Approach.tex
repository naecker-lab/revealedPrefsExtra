...
\begin{align*}\sup \mathcal{E}_{(\mathcal{X},\mathcal{P})} &= \left(\frac{1}{\min\left\{\sup \mathcal{E}^V_{(\mathcal{X},\mathcal{P})}\ |\ V\in\mathcal{V}\right\}}\right)^{-1} \\ &= \left(\max\left\{\inf
\left(\frac{1}{\mathcal{E}^V_{(\mathcal{X},\mathcal{P})}}\right)\ |\ V\in\mathcal{V}\right\}\right)^{-1}
\end{align*}
Note then that, $\forall V\in\mathcal{V}$, we have $\sup \mathcal{E}^V_{(\mathcal{X},\mathcal{P})} = \frac{P_iX_j}{P_iX_i}$ for some $i,j\in\{1,\ldots,k\}$ where $i\not=j$ since either $\sup \mathcal{E}^V_{(\mathcal{X},\mathcal{P})}$ is supposed to remove an edge on $G_s$ or, $\forall \epsilon>0$, $\sup \mathcal{E}^V_{(\mathcal{X},\mathcal{P})} - \epsilon$ is supposed to remove a walk (by removing some edge along it) on $G_d$. Hence, $\forall V\in\mathcal{V}$, $\left(\sup \mathcal{E}^V_{(\mathcal{X},\mathcal{P})}\right)^{-1}$ is an element of $\Omega_d$ or $\Omega_s$ since it's of the form $\frac{P_iX_i}{P_iX_j}$. Now we begin to construct the algorithm for Afriat's CCEI.
...
\]
\end{restatable}
The claim of Theorem \ref{thm:CCEI_main} can be better explained with the following illustration. Let $t=3$ and uppose that for $X_i,X_j\in\mathcal{X}$, the only walks that connect $X_i$ to $X_j$ with 3 edges are as follows:
\begin{figure}[H]
\centering
\begin{tikzpicture}[scale = 1.5]
\tikzset{
